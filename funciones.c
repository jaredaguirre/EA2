...
int compararMovim_MIO(const void *d1, const void *d2)
{
    ...
    a entero usando la funcion de biblioteca atoi, ya que ni siquiera
    entraria en una variable de tipo long!
...
void ordenarLista_MIO(tLista *p, int (*compararMovim_MIO)(const void *, const void *))
{
    ...
    */
    ...
    while(pri->sig)
    {
        ...
        while(r)
        {
            if(compararMovim_MIO && compararMovim_MIO((r)->info, menor->info) < 0 )
                menor = r;
            r = r->sig;
        }
